...
export default function DashboardLayout({ children }) {
    ...
    useEffect(() => {
        ...
        const verify = async () => {
            try {
                ...
                // If non-superadmin tries to access Overview, redirect to Templates
                if (data.user.role !== 'superadmin' && pathname === '/dashboard') {
    ...
    const navItems = [
        ...
        },
        {
            ...
            superadminOnly: true,
            icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 21v-4a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4v4"></path><circle cx="12" cy="7" r="4"></circle><path d="M12 11V3"></path><path d="M8 7l4-4 4 4"></path></svg>
        },
        {
            name: 'Team',
            path: '/dashboard/team',
        ...
        <div className="flex h-screen overflow-hidden bg-background text-foreground transition-all duration-300">
            <aside className={`${isCollapsed ? 'w-20' : 'w-72'} border-r border-border p-4 flex flex-col bg-card z-30 shrink-0 h-full transition-all duration-300`}>
                <div className={`flex items-center justify-between mb-12 ${isCollapsed ? 'flex-col gap-4' : ''}`}>
                    <Link href={user?.role === 'superadmin' ? "/dashboard" : "/dashboard/templates"} className="overflow-hidden transition-all duration-300">
                        <h2 className="text-3xl font-bold text-primary tracking-tight font-header whitespace-nowrap">
                            {isCollapsed ? 'V' : 'VerifyCert'}
                        </h2>
                        {!isCollapsed && user?.organization?.name && (
                ...
                <nav className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar overflow-x-hidden">
                    {navItems
                        .filter(item => {
                            ...
                            if (item.orgAdminOnly && !['owner', 'admin'].includes(user?.orgRole)) return false;
                            return true;
                        })
                        .map((item) => (
                            <Link
