...
// ═══ Template Edit: Get Template Content as HTML ═══
router.get('/templates/:id/html', auth, async (req, res) => {
    try {
        const template = await Template.findById(req.params.id);
        if (!template) return res.status(404).json({ error: 'Template not found' });
        ...
        console.error('Template HTML conversion error:', err);
        res.status(500).json({ error: err.message });
    }
});

// ═══ Template Edit: Save Edited Content Back as DOCX ═══
router.put('/templates/:id/content', auth, async (req, res) => {
    try {
        const { html } = req.body;
        ...
        // Strategy: Create a proper HTML document wrapper, convert to DOCX via LibreOffice
        const fullHtml = `
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        ...
        blockquote { border-left: 3px solid #ccc; padding-left: 12pt; margin: 10pt 0; color: #555; }
...
${html}
        ...
        const tempHtmlPath = path.join(__dirname, `../uploads/temp_edit_${Date.now()}.html`);
        ...
        let sofficePath = possiblePaths.find(p => fs.existsSync(p)) || 'soffice';
        const cmd = `"${sofficePath}" --headless --convert-to docx --outdir "${outputDir}" "${tempHtmlPath}"`;
        ...
        const baseName = path.basename(tempHtmlPath, '.html');
        const newDocxPath = path.join(outputDir, `${baseName}.docx`);

        if (!fs.existsSync(newDocxPath)) {
            // Cleanup temp html
            if (fs.existsSync(tempHtmlPath)) fs.unlinkSync(tempHtmlPath);
            return res.status(500).json({ error: 'DOCX conversion failed. LibreOffice could not create the file.' });
        }
        ...
        // Move new DOCX to the template's location
        const finalDocxPath = path.join(outputDir, `${Date.now()}-${template.name.replace(/[^a-zA-Z0-9]/g, '_')}.docx`);
        fs.renameSync(newDocxPath, finalDocxPath);

        // Re-extract placeholders from the new DOCX
        const newPlaceholders = await extractPlaceholders(finalDocxPath);

        // Regenerate thumbnail
        let newThumbnailPath = template.thumbnailPath;
        try {
            ...
            const outputId = Date.now() + '-preview';
            newThumbnailPath = await generateThumbnail(finalDocxPath, outputId);
        } catch (thumbErr) {
            console.error('Thumbnail regeneration failed:', thumbErr.message);
            // Non-fatal, keep old thumbnail path or null
        }

        // Update template record
        template.filePath = finalDocxPath;
        template.placeholders = newPlaceholders;
        template.thumbnailPath = newThumbnailPath;
        await template.save();

        res.json({
            success: true,
            message: 'Template saved as .docx successfully',
            template: {
                _id: template._id,
                name: template.name,
                filePath: template.filePath,
        ...
        console.error('❌ Template content save error:', err);
        res.status(500).json({ error: err.message });
    }
});
